...
static int trans_vqdmullb(DisasContext *s, arg_2op *a)
{
    static MVEGenTwoOpFn *const fns[] = {
        NULL,
...
static int trans_vqdmullt(DisasContext *s, arg_2op *a)
{
    static MVEGenTwoOpFn *const fns[] = {
        NULL,
...
int process_interrupt(int interrupt_request, CPUState *env)
{
    ...
    bool exception_is_eligible = true;
#ifdef TARGET_PROTO_ARM_M
    const bool exception_mask_enabled = (env->v7m.primask[M_REG_S] & PRIMASK_EN)
        || (env->v7m.primask[M_REG_NS] & PRIMASK_EN)
        || (env->v7m.faultmask[M_REG_S] & 1)
        || (env->v7m.faultmask[M_REG_NS] & 1);
    if((interrupt_request & CPU_INTERRUPT_HARD) && exception_mask_enabled) {
        int pending_exception = tlib_nvic_find_pending_irq();
        exception_is_eligible = tlib_nvic_is_pending_irq_eligible(pending_exception);
    }
#endif
    if((interrupt_request & CPU_INTERRUPT_HARD) &&
#ifdef TARGET_PROTO_ARM_M
       (env->regs[15] < 0xffffffe0) && exception_is_eligible)
#else
